...
class GuiMainMenu : AbstractScreen() {
    ...
    private fun showWelcomePopup() {
        popup = PopupScreen {
            title("§a§lWelcome!")
            message("""
                §eWelcome to the Shade Client!
        
                §6Let's Hacking on Blocksmc!
                §aTheres have so many modules!
                §aRight Shift To open the GUI!
                §ayou can bind modules to .bind (modulename) (key) !
        
                §6Important Commands:§r
                §a- §f.bind <module> <key> / .bind <module> none
                §a- §f.onlineconfig load <name> / .onlineconfig list
            """.trimIndent())
            button("§aBlocksmc = so many fat staffs")
            onClose { popup = null }
        }
    }
